...
The browser package registers an additive action in `conversation.session.header.actions` and supplies the only active `root` occupant while this Bundle is present. That root preserves the original sidebar, conversation, details, and overlay slots and renders the workbench directly; it does not add a public workbench slot. Responsive Flex columns and in-flow resize handles replace fixed CSS tracks; stored numeric sizes remain mutable drag preferences constrained against the observed viewport. Workbench visibility is browser-local and isolated by selected Session, so an unseen Session starts closed and returning to a Session restores its prior visibility. Restoring a Session page may mount its iframe inside the hidden, inert section without opening the right-hand column; after an explicit open, closing retains the iframe so reopening does not reload it. The workbench header's fullscreen control overlays Label Studio across the DSH page; selecting it again, pressing `Escape`, or closing the workbench restores the docked layout. Reload replaces only the iframe, **Open in a new window** uses the configured endpoint, and closing the workbench leaves both the conversation and Label Studio server running.
