...
> 状态：设计稿，尚未实现。本文中的路径全部相对于独立插件目录 `/Users/xinlongzhang/PycharmProjects/dsh-label-studio-plugin-package`；任何实现都不得修改 `/Users/xinlongzhang/PycharmProjects/deepseek-harness`。
...
| Label Studio Webhook | 已认证事件与已有绑定精确匹配，或事件确认已绑定资源被删除 | 保持现有目标，或清除、降级已失效绑定；不得建立新绑定 |
...
- Webhook 不携带 DSH Session ID，也不能证明事件来自 DSH iframe；活动 Session 数量不得用作归属依据。
- 没有已有精确绑定的 Webhook 创建或更新事件只报告 `unassigned`，不得建立或替换任何 Session 绑定。
...
| `src/webhook-binding.ts` | 新增 | 只同步已有精确绑定和已删除资源 |
...
| `src/runtime.ts` | 修改 | managed 模式启用 organization webhook 并配置投递超时 |
...
export interface LabelStudioBindingSnapshot {
...
`commitBinding()` 复用现有 Store 的 per-Session queue，并对 binding revision 执行 CAS；它不修改页面 revision。项目删除会清除所有指向该项目的绑定，task 删除只把指向被删除 task 的绑定降级为 project，不影响其他 Session。

现有页面 snapshot、页面提交回执和页面恢复接口保持不变。`binding` 在旧记录中缺失时解析为空 snapshot，从而不需要第二个 storage domain 或迁移程序；被动 `page/commit` 只能更新页面状态，不能更新 binding。现有 domain 增加一个单例 owner table，`ensureWebhookOwnerId()` 原子保留首次写入的随机 owner ID，且不读取或修改任何 Session record。
...
Client 必须同时校验 `event.source === iframe.contentWindow`、`event.origin === frameBaseUrl.origin`、协议常量、inspection ID、当前 Session epoch 和 deadline。Host 收到回执后再次验证 lease/generation，并使用 REST 确认 task 属于 project；`projects` 页面不能满足 project/task 操作。
...
同一 Session 同时只允许一个检查请求。Session、lease、Connection generation 或插件生命周期发生变化时，旧请求必须失败，不能把旧 iframe 页面提交给新 Session。
...
export class LabelStudioFrameProxy {
  start(signal?: AbortSignal): Promise<LabelStudioFrameProxyAddress>
  close(): Promise<void>
}
...
代理只监听 loopback 随机端口，只转发到经过配置校验的 Label Studio loopback origin。它重写 upstream `Location`、`Origin` 和 `Referer`，并把固定 Bridge 脚本作为同源静态资源注入 HTML；它不得转发到请求参数指定的任意主机，也不得放宽 Label Studio 的 CSP。

`htmlMaxBytes` 只限制需要缓冲和注入的 HTML。上传请求以及图片、音频、视频、导出文件和其他非 HTML 响应使用背压感知的流式透传，不受该限制；压缩 HTML 必须在注入前安全解码并更新响应编码与长度。
...
export type LabelStudioWebhookEvent =
  ...
  | { readonly action: 'TASK_CREATED' | 'TASKS_CREATED'; readonly projectId: LabelStudioProjectId; readonly taskIds: readonly LabelStudioTaskId[] }
  | { readonly action: 'TASK_DELETED' | 'TASKS_DELETED'; readonly projectId: LabelStudioProjectId; readonly taskIds: readonly LabelStudioTaskId[] }
  | { readonly action: 'ANNOTATION_CREATED' | 'ANNOTATION_UPDATED'; readonly projectId: LabelStudioProjectId; readonly taskIds: readonly [LabelStudioTaskId]; readonly annotationIds: readonly [LabelStudioAnnotationId] }
  | { readonly action: 'ANNOTATIONS_CREATED'; readonly projectId: LabelStudioProjectId; readonly taskIds: readonly LabelStudioTaskId[]; readonly annotationIds: readonly LabelStudioAnnotationId[] }
  | { readonly action: 'ANNOTATION_DELETED' | 'ANNOTATIONS_DELETED'; readonly projectId: LabelStudioProjectId; readonly annotationIds: readonly LabelStudioAnnotationId[] }
...
`ensureInstalled()` 先读取当前 Label Studio 支持的 action，再创建启用 payload 的 organization webhook。请求必须显式提交 `is_active: true`、`project: null`、`send_for_all_actions: false` 和 `send_payload: true`，并设置随机认证 secret 与持久 owner ID 两个独立 header；插件不得复用 PAT 作为任一值。

可用 action 必须覆盖 project 创建/更新/删除、task 创建/删除、annotation 创建/更新/删除八个语义类别；单条与批量名称属于同一类别。注册器订阅各类别中服务端实际返回的已知名称，缺少任一类别时按 `webhookMode` 报 required 失败或 optional unavailable，不发送服务端未声明的 action。

owner ID 在插件现有 storage domain 的单例记录中生成并持久化，不进入 Session binding。启动时只对账 owner header 完全匹配的 Webhook：删除残留后创建一个新注册；创建或删除结果未知时重新列举并按 owner ID 精确收敛，禁止仅凭 callback URL 删除用户创建的 Webhook。

managed Python 模式在启动进程时显式提供 `LABEL_STUDIO_ALLOW_ORGANIZATION_WEBHOOKS=true` 和配置解析后的 `WEBHOOK_TIMEOUT`。external 模式不能修改外部进程；`webhookMode: 'required'` 时注册失败导致插件加载失败，`optional` 时工具和按需 Bridge 保持可用，但 UI 必须显示 Webhook unavailable。
...
Webhook 创建和更新事件不得建立新绑定或替换目标。项目和 task import 事件只匹配相同 project 的已有绑定，annotation 创建或更新事件只匹配相同 project/task 的已有 task binding；匹配只确认事件与现有上下文一致，不修改 target、source、boundAt 或 revision。
...
没有精确匹配的创建或更新事件返回 `unassigned`，并向当前插件租约广播不含 project/task/annotation 数据的状态事件；它不创建 Session、不缓存 payload，也不在之后补猜归属。多个 Session 已有相同精确绑定时可以同时报告匹配，因为 target 不发生变化。
...
`baseUrl` 只供外部打开链接和显示使用；iframe 使用 `frameBaseUrl`。启动配置不得包含 PAT、access token、Webhook secret、Session ID 或 Label Studio 数据。
...
| 创建或更新 Webhook 没有精确匹配 | 任意 | 不变并报告 unassigned |
...
5. Webhook 只确认已有精确绑定或同步资源删除；独立 8080 页面、其他浏览器和外部 API 产生的事件不能建立 DSH Session 绑定。
6. 卸载插件并重启后原布局恢复，DSH 源码、Session 日志和 prompt 结果不发生结构变化。
7. 重新安装插件后从 `label_studio_context` 恢复 binding；启动对账确保同一 owner ID 只有一个 Webhook、一个代理和一个 route。
