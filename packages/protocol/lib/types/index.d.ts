...
export type LabelStudioBrowserEvent = LabelStudioInspectPageEvent | LabelStudioWebhookUnassignedEvent | LabelStudioBindingChangedEvent | LabelStudioWebhookStatusEvent | {
...
export type LabelStudioBindingErrorCode = 'binding-missing' | 'binding-conflict' | 'binding-target-mismatch' | 'current-page-unavailable' | 'current-page-timeout' | 'current-page-unsupported' | 'webhook-unavailable' | 'webhook-unassigned';
/** Stable Label Studio failures nested inside the transport RPC result. */
export type LabelStudioRpcErrorCode = 'invalid-request' | 'session-not-found' | 'lease-conflict' | 'lease-expired' | 'stale-generation' | 'stale-revision' | 'future-revision' | 'focus-conflict' | 'focus-not-found' | LabelStudioSessionContextErrorCode | LabelStudioBindingErrorCode;
