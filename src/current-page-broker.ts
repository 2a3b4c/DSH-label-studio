...
import { labelStudioPageInspectionId } from './context-types.ts'
import type { LabelStudioSessionIdentity } from './session-context-spec.ts'

type CurrentPageErrorCode = Extract<
  LabelStudioBindingErrorCode,
...
export class LabelStudioCurrentPageError extends Error {
   ...
   */
  constructor(readonly code: CurrentPageErrorCode, message: string) {
    super(message)
...
interface PendingInspection {
  ...
  readonly resolve: (page: LabelStudioPageContext) => void
...
interface CompletedInspection {
  ...
  readonly generation: number
  readonly inspectionId: LabelStudioPageInspectionId
  readonly outcome: LabelStudioInspectPageCommit['outcome']
}

interface InspectionState {
  pending: PendingInspection | undefined
  completed: CompletedInspection | undefined
}

/** Coordinates one concurrent current-page inspection per DSH Session. */
export class LabelStudioCurrentPageBroker {
  private readonly states = new Map<SessionId, InspectionState>()
  ...
  constructor(
    ...
    private readonly changes: Pick<LabelStudioChangeBroker, 'publishCurrentPageInspection'>,
  ...
  request(
    ...
    if (binding === undefined) return Promise.reject(unavailable('this Session has no active browser lease'))
    ...
    const promise = new Promise<LabelStudioPageContext>((resolve, reject) => {
   ...
   */
  commit(
    commit: LabelStudioInspectPageCommit,
    identity: LabelStudioSessionIdentity,
    ...
    if (completed !== undefined && sameCommit(completed, commit)) return { accepted: true }
    ...
    switch (commit.outcome.kind) {
      case 'page': pending.resolve(commit.outcome.page); break
      case 'unavailable': pending.reject(unavailable('the Label Studio iframe is unavailable')); break
      case 'unsupported': pending.reject(new LabelStudioCurrentPageError(
        'current-page-unsupported', 'the current Label Studio route is unsupported',
      )); break
    }
  ...
  private finish(sessionId: SessionId, inspectionId: LabelStudioPageInspectionId, settle: () => void): void {
    const state = this.states.get(sessionId)
...
function unavailable(message: string): LabelStudioCurrentPageError {
  return new LabelStudioCurrentPageError('current-page-unavailable', message)
}

function sameCommit(completed: CompletedInspection, commit: LabelStudioInspectPageCommit): boolean {
  if (completed.leaseId !== commit.leaseId
    || completed.generation !== commit.generation
    || completed.inspectionId !== commit.inspectionId
    || completed.outcome.kind !== commit.outcome.kind) return false
