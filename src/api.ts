...
export class LabelStudioApi {
  ...
  private async request<T>(path: string, request: {
    ...
    }, request.method === 'POST') as Promise<T>
  }
  ...
  private async fetchJsonObject(
    ...
    if (!response.ok) throw new LabelStudioHttpError(init.method as 'GET' | 'POST', path, response.status)
    try {
...
function decodeMutationResponse<T>(operation: string, decode: () => T): T {
  try {
    return decode()
  } catch {
...
function parseJsonObject(raw: string, operation: string): Record<string, unknown> {
  let value: unknown
  try {
