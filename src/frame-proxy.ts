...
export class LabelStudioFrameProxy {
  ...
  private address: LabelStudioFrameProxyAddress | undefined
  private starting: Promise<LabelStudioFrameProxyAddress> | undefined
  private closePromise: Promise<void> | undefined
  private closing = false

  /** @param options - fixed loopback upstream, protocol, and decoded HTML limit. */
  constructor(private readonly options: LabelStudioFrameProxyOptions) {
  ...
  start(): Promise<LabelStudioFrameProxyAddress> {
    ...
    if (this.closing) return Promise.reject(new Error('label-studio: frame proxy is closed'))
    this.starting = new Promise<LabelStudioFrameProxyAddress>((resolve, reject) => {
  ...
  private async stop(): Promise<void> {
    ...
    for (const request of this.upstreamRequests) request.destroy(new Error('frame proxy closed'))
  ...
  private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
      ...
      if (address === undefined || request.headers.host !== new URL(address.origin).host) return fail(response, 400)
      if (request.method === 'CONNECT' || request.headers.upgrade !== undefined) return fail(response, 405)
      ...
      if (headers === undefined) return fail(response, 403)
  ...
  private upstreamHeaders(
    ...
    if (origin !== undefined) {
      if (origin !== proxyOrigin && !safeMethod(method)) return undefined
      headers.origin = origin === proxyOrigin ? this.upstream.origin : origin
    }
    const referer = singleHeader(source.referer)
    if (referer !== undefined) {
      let parsed: URL
      try { parsed = new URL(referer) } catch { return undefined }
      if (parsed.origin !== proxyOrigin && !safeMethod(method)) return undefined
  ...
  private forward(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      upstreamRequest.once('response', (upstreamResponse) => {
        void this.forwardResponse(
          upstreamResponse, outgoing, proxyOrigin, incoming.method ?? 'GET',
        ).then(resolve, reject)
      })
      void pipeline(incoming, upstreamRequest).catch((error: unknown) => {
  ...
  private async forwardResponse(
    ...
    const injected = Buffer.from(injectLabelStudioInspectionBridge(
      body.toString('utf8'), this.options.inspectionProtocol,
    ))
...
function requireHttpLoopbackOrigin(value: string): URL {
  let url: URL
  try { url = new URL(value) } catch { throw new TypeError('upstreamBaseUrl must be a loopback HTTP origin') }
...
function fixedRequestPath(value: string | undefined): string {
...
function filteredHeaders(source: IncomingHttpHeaders): IncomingHttpHeaders {
  ...
  for (const [name, value] of Object.entries(source)) {
    ...
    if (blocked.has(lower) || lower.startsWith('x-forwarded-') || value === undefined) continue
    target[lower] = value
  }
  return target
}
...
function safeMethod(method: string): boolean { return method === 'GET' || method === 'HEAD' || method === 'OPTIONS' }
function isHtml(value: string | undefined): boolean { return value?.split(';', 1)[0]?.trim().toLowerCase() === 'text/html' }
function hasBody(status: number): boolean { return status !== 204 && status !== 304 && status >= 200 }

function rewriteLocation(headers: IncomingHttpHeaders, upstreamOrigin: string, proxyOrigin: string): void {
...
async function collectDecodedHtml(
  ...
  const sink = new Writable({
    write(chunk, _encoding, callback) {
      ...
      size += value.length
      if (size > limit) callback(new Error('decoded HTML exceeds frameProxyHtmlMaxBytes'))
      else { chunks.push(value); callback() }
    },
  })
  if (decoder === undefined) await pipeline(source, sink)
