...
const labelStudioSessionContextDomainSpec = defineDomain({
	...
	version: 1,
	tables: { sessions: domainTable(labelStudioSessionContextRecordSchema) }
});
//#endregion
...
*/
function registerLabelStudioContextRpc(ctx, registry, broker, sessionContexts, operations, options) {
	let closing = false;
	const handler = async (rawEndpoint, payload, signal) => {
		...
		try {
			return outer(success(await operations.run(signal, (operationSignal) => dispatch(rawEndpoint, payload, operationSignal, ctx, registry, broker, sessionContexts, options))));
		} catch (error) {
...
async function dispatch(endpoint, payload, signal, ctx, registry, broker, sessionContexts, options) {
	switch (endpoint) {
		...
		case "focus/ack": {
			...
			await requirePersistentSession(ctx, registry.inspectLease(request.leaseId, request.generation).sessionId, signal, registry, broker, sessionContexts);
		...
		async execute(args, exec) {
			return operations.run(exec.signal, async (signal) => {
				...
				const committed = await changes.requestFocus({
					...
					createdAt: exec.agent.session.header.createdAt
				}, correlationId, reservation, target, policy.focusAckTimeoutMs, signal);
				return {
		...
		async execute(_args, exec) {
			return operations.run(exec.signal, async (signal) => {
				...
				try {
					...
					if (isMissingProjectResponse(error, active.target.projectId)) await changes.markProjectDeleted({
						sessionId: exec.agent.id,
						createdAt: exec.agent.session.header.createdAt
...
//#endregion
export { Config, DEFAULT_ACTIVE_TASK_MAX_BYTES, DEFAULT_CONTEXT_CLOSE_TIMEOUT_MS, DEFAULT_CONTEXT_LEASE_TTL_MS, DEFAULT_CONTEXT_OPEN_RETRY_MS, DEFAULT_EVENT_HISTORY_SIZE, DEFAULT_EVENT_WAIT_TIMEOUT_MS, DEFAULT_FOCUS_ACK_TIMEOUT_MS, DEFAULT_LABEL_STUDIO_BASE_URL, DEFAULT_LABEL_STUDIO_LAUNCH_MODE, DEFAULT_PYTHON_EXECUTABLE, DEFAULT_RECENT_PROJECT_LIMIT, DEFAULT_REFRESH_TOKEN_CREDENTIAL, DEFAULT_REST_RESPONSE_MAX_BYTES, DEFAULT_SHUTDOWN_GRACE_MS, DEFAULT_STARTUP_TIMEOUT_MS, LabelStudioApi, LabelStudioChangeBroker, LabelStudioContextError, LabelStudioContextRegistry, LabelStudioHttpError, LabelStudioMutationOutcomeUnknownError, LabelStudioOperationClosedError, LabelStudioOperationGate, LabelStudioRuntime, LabelStudioSessionContextError, LabelStudioSessionContextStore, apply, disposeLabelStudioResources, inject, injectLabelStudioBootConfig, labelStudioAnnotationId, labelStudioContextLeaseId, labelStudioContextSourceId, labelStudioFocusCorrelationId, labelStudioNavigationSequence, labelStudioPageContextSchema, labelStudioPredictionId, labelStudioProjectId, labelStudioSessionContextDomainSpec, labelStudioSessionContextRecordSchema, labelStudioTaskId, name, registerLabelStudioContextRpc, registerLabelStudioTools, resolveConfig, validateSelectedTask };
