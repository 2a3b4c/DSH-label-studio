...
import { labelStudioSessionContextDomainSpec, labelStudioWebhookOwnerRecordSchema, } from "./session-context-spec.js";
...
export class LabelStudioSessionContextStore {
    ...
    commitBinding(identity, request) {
        return this.enqueue(identity.sessionId, async () => {
            ...
            if (sameBindingRequest(current, request))
    ...
    async reconcileRecords(transform) {
        const changes = await Promise.all([...this.table.keys()].map(sessionId => this.enqueue(sessionId, async () => {
...
function sameBindingTarget(left, right) {
    if (left.kind !== right.kind || left.projectId !== right.projectId)
        return false;
...
function projectDeletedBinding(current, projectId) {
    ...
    if (!targetDeleted && !historyChanged)
        return undefined;
    const revision = current.revision + 1;
    if (targetDeleted)
...
function projectDeletedRecord(record, projectId) {
    ...
    if (!pageChanged && binding === undefined)
        return undefined;
    return {
...
function tasksDeletedRecord(record, projectId, taskIds, now) {
    ...
    if (!pageChanged && binding === undefined)
        return undefined;
    return {
        record: reconciledRecord(record, pageUsesTask ? { view: 'project', projectId } : record.page, recentProjects, pageChanged, binding),
        bindingChanged: binding !== undefined,
    };
}
