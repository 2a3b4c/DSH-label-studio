...
import { presentActiveTaskCall, presentActiveTaskMeta, presentCreateActivePredictionCall, presentCreatePredictionCall, presentCreateProjectCall, presentFocusTaskCall, presentImportTasksCall, presentStatusCall, presentUpdateLabelConfigCall, } from "./present.js";
/**
 ...
 */
export function registerLabelStudioTools(ctx, runtime, api, contexts, changes, operations, resolver, bindings, policy) {
    const disposers = [];
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: (_args, exec) => operations.run(exec.signal, async (signal) => {
            ...
            if (status.available && policy.ensureWebhook !== undefined)
                await policy.ensureWebhook(signal);
            return status;
        }),
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            return operations.run(exec.signal, async (signal) => {
                ...
                const target = { kind: 'project', projectId: labelStudioProjectId(project.id) };
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        description: 'Import JSON tasks into an existing Label Studio project. Supply project_id for an explicit target, set '
        ...
        async execute(args, exec) {
            return operations.run(exec.signal, async (signal) => {
                ...
                const target = { kind: 'project', projectId: context.target.projectId };
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        description: 'Create a pre-annotation prediction for one Label Studio task. Supply task_id and optional project_id for an '
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        description: 'Create a pre-annotation prediction for this DSH Session task binding. Set current_page when the user refers '
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            return operations.run(exec.signal, async (signal) => {
                ...
                const warning = await commitWarning(resolver, identity, committedTarget, context.expectedBindingRevision);
                return {
                    projectId: committed.target.projectId,
                    taskId: committed.target.taskId,
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        description: 'Replace one Label Studio project labeling interface. Supply project_id for an explicit target, set '
            + 'current_page when the user refers to the visible iframe, or omit both to use this DSH Session binding. '
            ...
            return operations.run(exec.signal, async (signal) => {
                ...
                try {
                    ...
                    if (isMissingProjectResponse(error, target.projectId)) {
                        await changes.markProjectDeleted(identity, target.projectId);
                    }
                    throw error;
                }
...
function requireSessionIdentity(agent, operation) {
    if (agent === undefined)
        throw new Error(`label-studio: ${operation} requires a DSH Session`);
    return { sessionId: agent.id, createdAt: agent.session.header.createdAt };
}
function projectSelector(projectId, currentPage) {
    if (projectId !== undefined)
        return { mode: 'explicit', projectId: labelStudioProjectId(projectId) };
...
function requireTaskTarget(target) {
    if (target.kind !== 'task')
        throw new Error('label-studio: resolved target does not identify a task');
    return target;
}
async function commitWarning(resolver, identity, target, expectedBindingRevision) {
    const outcome = await resolver.commitSuccessfulResult(identity, target, 'tool-result', expectedBindingRevision);
